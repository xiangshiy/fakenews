...
def SVFEND_collate_fn(batch):
    ...
    title_inputid = [item['title_input_id'] for item in batch]
    ...
    frames = [item['resnet50_feature'] for item in batch]
    frames, frames_masks = pad_frame_sequence(num_frames, frames)

    audioframes = [item['vggish_feature'] for item in batch]
...
class Run():
    ...
    def get_dataloader(self, data_type, data_fold):
        ...
        if data_type == 'SVFEND':
            dataset_train = SVFENDDataset('D:\Github\FakeSV\dataset\\video_id_train.txt')
            dataset_test = SVFENDDataset('D:\Github\FakeSV\dataset\\video_id_test.txt')
            collate_fn = SVFEND_collate_fn
        # elif data_type == 'FANVM':
        #     dataset_train = FANVMDataset_train(f'vid_fold_no_{data_fold}.txt')
        #     dataset_test = FANVMDataset_test(path_vid_train=f'vid_fold_no_{data_fold}.txt',
        #                                      path_vid_test=f'vid_fold_{data_fold}.txt')
